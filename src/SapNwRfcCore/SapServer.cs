...
public sealed class SapServer : ISapServer
{
    ...
    private SapServer(RfcInterop interop, IntPtr rfcServerHandle, SapConnectionParameters parameters, Action<ISapServerConnection, ISapServerFunction> action)
    {
        ...
        var resultCode = _interop.AddServerSessionChangedListener(
    ...
    private static SapServer Create(RfcInterop rfcInterop, SapConnectionParameters parameters, Action<ISapServerConnection, ISapServerFunction> action)
    {
        var interopParameters = parameters.ToInterop();
    ...
    public event EventHandler<SapServerErrorEventArgs> Error
    {
        add
        {
            if (_error == null)
            {
                var resultCode = _interop.AddServerErrorListener(
    ...
    public event EventHandler<SapServerStateChangeEventArgs> StateChange
    {
        add
        {
            ...
            {
                var resultCode = _interop.AddServerStateChangedListener(
                    rfcHandle: _rfcServerHandle,
    ...
    {
        var resultCode = _interop.GetServerAttributes(
            rfcHandle: _rfcServerHandle,
            serverAttributes: out var serverAttributes,
            errorInfo: out var errorInfo);
    ...
    public void Launch()
    {
        var resultCode = _interop.LaunchServer(
            rfcHandle: _rfcServerHandle,
            errorInfo: out var errorInfo);
    ...
    {
        var resultCode = _interop.ShutdownServer(
            rfcHandle: _rfcServerHandle,
            timeout: timeout,
            errorInfo: out var errorInfo);
...
#pragma warning restore SA1306 // Field names should begin with lower-case letter
    ...
    private static void InstallGenericServerFunctionHandler(RfcInterop interop, Func<string, SapAttributes, ISapFunctionMetadata> getFunctionMetadata)
    {
        ...
        {
            var resultCode = interop.InstallGenericServerFunction(
                serverFunction: ServerFunction,
                funcDescPointer: GenericMetadata,
                out var installFunctionErrorInfo);
    ...
    {
        (var interop, _) = GenericServerFunctionHandler;
        if (interop == null)
        {
        ...
        var serverContextResultCode = interop.GetServerContext(
            rfcHandle: connectionHandle,
            serverContext: out var serverContext,
            errorInfo: out var serverContextErrorInfo);
        ...
        {
            var setServerStatefulResultCode = interop.SetServerStateful(
                rfcHandle: connectionHandle,
                isStateful: 1,
                errorInfo: out var setServerStatefulErrorInfo);
        ...
        if (Servers.TryGetValue(serverContext.SessionId, out var sapServer))
        {
            IntPtr functionDesc = interop.DescribeFunction(
                rfcHandle: functionHandle,
                errorInfo: out var functionDescErrorInfo);
    ...
    private static RfcResultCode HandleGenericMetadata(string functionName, RfcAttributes attributes, out IntPtr funcDescHandle)
    {
        ...
        (var interop, var getFunctionMetadata) = GenericServerFunctionHandler;
        if (interop == null || getFunctionMetadata == null)
            return RfcResultCode.RFC_NOT_FOUND;

        try
        {
            var functionMetadata = getFunctionMetadata(functionName, new SapAttributes(attributes));
