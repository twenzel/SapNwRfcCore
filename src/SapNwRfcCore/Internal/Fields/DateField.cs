...
internal sealed class DateField : Field<DateTime?>
{
    ...
    public override void Apply(RfcInterop? interop, IntPtr dataHandle)
    {
        ...
        var resultCode = interop!.SetDate(
    ...
    public static DateField Extract(RfcInterop? interop, IntPtr dataHandle, string name)
    {
        ...
        var resultCode = interop!.GetDate(
            dataHandle: dataHandle,
            name: name,
            emptyDate: buffer,
            errorInfo: out var errorInfo);
        ...
        if (!DateTime.TryParseExact(dateString, RfcDateFormat, CultureInfo.InvariantCulture, DateTimeStyles.None, out var date))
            return new DateField(name, null);
