...
internal sealed class DynamicRfcStructure : DynamicObject, IReadOnlyDictionary<string, object?>
{
    ...
    {
        if (_typeMetadata != null && _typeMetadata.Fields.TryGetValue(name, out var field) && field != null)
        {
    ...
    private bool TryGetByIndex(int index, Type returnType, out object? result)
    {
        if (_typeMetadata != null)
        {
            ...
            {
                var field = _typeMetadata.Fields[index];
    ...
    public override bool TryGetIndex(GetIndexBinder binder, object[] indexes, out object? result)
    {
        ...
        throw new ArgumentException("Invalid index", nameof(indexes));
    }
    ...
    public IEnumerable<string> Keys
    {
        get
        {
            if (_typeMetadata != null)
            {
                foreach (var f in _typeMetadata.Fields)
                    yield return f.Name;
            }
        }
    }
    ...
    public IEnumerable<object?> Values
    {
        get
        {
            if (_typeMetadata != null)
            {
                foreach (var f in _typeMetadata.Fields)
                {
                    if (TryGetByName(f.Name, typeof(object), out var result))
                    {
                        yield return result;
                    }
                }
    ...
    public int Count => _typeMetadata?.Fields.Count ?? 0;
    ...
    public IEnumerator<KeyValuePair<string, object?>> GetEnumerator()
    {
        if (_typeMetadata != null)
        {
            ...
            {
                if (TryGetByName(field.Name, typeof(object), out var result))
                {
